...
from crs4.cassandra_utils import CassandraUnsupervisedWriter
...
def get_jobs(src_dir, splits=["train", "val"]):
    ...
    if splits:
        for or_split in splits:
            ...
            partition_items = (or_split)
            fns = os.listdir(sp_dir)
            for fn in fns:
                path = os.path.join(cur_dir, fn)
                jobs.append((path, partition_items))
    else:
        partition_items = ()
        cur_dir = os.path.join(src_dir)
...
def send_images_to_db(
    ...
    def ret(jobs):
        cw = CassandraUnsupervisedWriter(
            cass_conf=cass_conf,
        ...
        for path, partition_items in tqdm(jobs):
            cw.enqueue_image(path, partition_items)
        cw.send_enqueued()
